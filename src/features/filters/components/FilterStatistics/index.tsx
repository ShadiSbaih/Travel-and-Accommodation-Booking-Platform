...
import { Alert, Box, Chip, Typography, Stack, Fade } from "@mui/material";
...
const FilterStatistics: React.FC<FilterStatisticsProps> = ({
  ...
  const { selectedAmenities, filterMode } = useAppSelector(
    (state) => state.filters
  );
        ...
        <Box>
          <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
            ...
            <Typography variant="body2" fontWeight={600} color="white">
              Found {filteredCount} hotel{filteredCount !== 1 ? "s" : ""} out of{" "}
              {totalCount} total
              {selectedAmenities.length > 1 &&
                ` with ${
                  filterMode === "any" ? "any" : "all"
                } of the selected amenities`}
